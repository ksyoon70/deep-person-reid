...
class OcrImageDataManager(DataManager):
    r"""
    OcrImageDataManager
    ...
    def __init__(
        ...
        super(OcrImageDataManager, self).__init__(
            sources=sources,
            targets=targets,
            height=height,
            width=width,
        ...
        if load_train_targets:
            ...
            for name in self.targets:
                ...
                print(f"DEBUG: Loaded target dataset '{name}', type: {type(trainset_t_)}")
                trainset_t.append(trainset_t_)
            
            if len(trainset_t) > 0:
                combined_trainset_t = trainset_t[0]
                for i in range(1, len(trainset_t)):
                    combined_trainset_t += trainset_t[i]
                trainset_t = combined_trainset_t
            else:
                raise RuntimeError("No target datasets loaded")
        ...
        print('=> OcrImageDataManager init complete')
